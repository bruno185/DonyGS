...
| **compute_face_plane_float** | Même calcul que `compute_face_plane` mais en flottant (compatible avec les distances FLOAT_TO_FLOAT). |
...
| **painter_geoV3** | Variante du painter géométrique inspirée de l’algorithme de R. Dony (sans split de faces). |
...
| **pair_plane_geometric_tests** | Version diagnostique interactive des tests planaires (affiche les détails sommet par sommet). |
| **point_in_poly_arrays_int** | Test point-in-polygon sur des tableaux d’entiers (ray casting 2D). |
| **point_in_poly_int** | Variante du test point-in-polygon. |
...
| **segs_intersect_int** | Teste l’intersection de deux segments 2D (coordonnées entières). |
| **segs_intersect_int_fixed64** | Même test d’intersection de segments en arithmétique Fixed64. |
