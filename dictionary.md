...
| **ray_cast_hierarchical** | Point d'entrée public (même nom, même signature qu'avant) : patch temporairement le scale de projection de f1/f2 via rch_patch_pair_at_scale (zoom artificiel x10), délègue à ray_cast_hierarchical_impl, puis restaure l'état via rch_unpatch_pair. |
| **ray_cast_hierarchical_impl** | Corps original de ray_cast_hierarchical (inchangé) : essaie centroïde QD, centroïde SH, puis centre de bbox. Appelé par le wrapper ray_cast_hierarchical après le patch de zoom. |
| **rch_gather_pair_vertex_indices** | Collecte les indices de sommets uniques (0-based, conversion depuis la convention OBJ 1-based) utilisés par une paire de faces f1/f2, en dédoublonnant les sommets partagés (faces adjacentes). |
| **rch_patch_pair_at_scale** | Reprojette temporairement les sommets uniques de f1/f2 à un scale de projection donné (zoom artificiel), écrit directement dans vtx->x2d/y2d, met à jour le cache de bbox 2D des deux faces, et sauvegarde tout l'état original pour restauration via rch_unpatch_pair. |
| **rch_recompute_face_bbox_cache** | Recalcule le cache de bbox 2D (faces->minx/maxx/miny/maxy) d'une face à partir de ses coordonnées vtx->x2d/y2d courantes ; appelé après un patch de zoom pour garder ce cache cohérent avec les coordonnées patchées. |
| **rch_unpatch_pair** | Restaure l'état original (coordonnées 2D des sommets, cache de bbox par face, scale de projection global) après un test effectué avec rch_patch_pair_at_scale. |
