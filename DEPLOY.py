...
def main():
    ...
    # Push the disk image to the Android device via ADB now that deployment succeeded.
    adb_command = f'adb push "{AppleDiskPath}" /sdcard/kegs/Live.Install.po'
    print(f"Pushing disk image to phone: {adb_command}")
    if not run_command(adb_command, "Pushing disk image to phone"):
        print("WARNING: adb push failed. The deployment to the phone was not completed.")
